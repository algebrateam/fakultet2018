...
class ConfigFakultetTest extends TestCase
{
    ...
    public function testAppKeyPostavljen()
    {
        ...
        $this->assertEquals('base64', substr($_ENV['APP_KEY'], 0, 6), 'Kljuc nije ispravan, mora pocinjati sa "base64"');

        $this->assertEquals(51, strlen($_ENV['APP_KEY']), 'Kljuc nije dovoljne duljine, mora imati 51 znak');
    }
    ...
    public function testPostavkeServiceProvider()
    {
        ...
        $this->markTestSkipped('Ovo je zastarjelo u verziji Laravel > 5.5');
        $this->assertTrue(in_array('Collective\Html\HtmlServiceProvider', config('app.providers')), '"Collective\Html\HtmlServiceProvider" postavi u "/config/app.php"');
    ...
    {
        $this->markTestSkipped('Ovo je zastarjelo u verziji Laravel > 5.5');
        $this->assertTrue(in_array('Collective\Html\FormFacade', config('app.aliases')), '"Alias Collective\Html\FormFacade" postavi u "/config/app.php"');

        $this->assertTrue(in_array('Collective\Html\HtmlFacade', config('app.aliases')), '"alias Collective\Html\HtmlFacade" postavi u "/config/app.php"');

        $this->assertTrue(in_array('Illuminate\Support\Facades\Input', config('app.aliases')), '"alias Illuminate\Support\Facades\Input" postavi u "/config/app.php"');

        $this->assertTrue(in_array('Illuminate\Support\Facades\App', config('app.aliases')), '"alias Illuminate\Support\Facades\App" postavi u "/config/app.php"');
    }
    ...
    {
        $this->assertTrue(in_array($alias, config('app.aliases')), '"alias '.$alias.'" postavi u "/config/app.php"');
    }
